...
function validateProgramme(programme, index, validChannelIds) {
  ...
  if (!programme['@_channel']) {
    ...
    warnings.push(
      `Programme at index ${index} references unknown channel "${programme['@_channel']}"`
    );
  }
  ...
  if (!programme['@_start']) {
    ...
    errors.push(
      `Programme at index ${index} has invalid "start" time format: ${programme['@_start']}`
    );
  }
  ...
  if (!programme['@_stop']) {
    ...
    errors.push(
      `Programme at index ${index} has invalid "stop" time format: ${programme['@_stop']}`
    );
  }
  ...
  if (!programme.title) {
    warnings.push(
      `Programme at index ${index} for channel "${programme['@_channel']}" missing title`
    );
  }
...
export function validateEPGCoverage(xmlString, channels) {
  ...
  try {
    ...
    result.coverage = {
      ...
      percentage:
        channels.length > 0 ? Math.round((channelsWithEPG.length / channels.length) * 100) : 0,
    ...
    if (channelsWithoutEPG.length > 0) {
      result.warnings.push(`${channelsWithoutEPG.length} channel(s) do not have EPG data`);
    }
  } catch (err) {

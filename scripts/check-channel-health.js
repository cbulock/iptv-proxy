...
async function checkStream(url) {
  ...
  try {
    ...
    return {
      ...
      ttfb,
      bytesRead,
      error: healthy ? '' : 'unhealthy content-type or no data',
      method: 'GET',
    };
  } catch (err) {
    return {
...
export async function runHealthCheck() {
  ...
  const regularTasks = regularChannels.map(channel =>
    limit(async () => {
      const id = channel.tvg_id || channel.guideNumber || channel.name;
  ...
  const meta = {
    ...
    totals: {
      online: detailed.filter(d => d.healthy).length,
      offline: detailed.filter(d => !d.healthy).length,
      total: detailed.length,
    },
  };
  await fs.writeFile(
    STATUS_FILE,
    JSON.stringify({ summary: statusMap, details: detailed }, null, 2)
  );

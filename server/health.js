...
router.get('/api/channel-health', requireAuth, channelHealthLimiter, async (req, res) => {
  try {
    let raw = {};
    try {
      raw = JSON.parse(await fs.readFile(STATUS_FILE, 'utf8'));
    } catch (err) {
      if (err.code !== 'ENOENT') console.warn('[Health] Could not read status file:', err.message);
    }
    const summary = raw.summary || raw; // backward compat
    const details = raw.details || [];
    let meta = null;
    try {
      const last = JSON.parse(await fs.readFile(LAST_LOG_FILE, 'utf8'));
      meta = last.meta || null;
    } catch (err) {
      if (err.code !== 'ENOENT')
        console.warn('[Health] Could not read last log file:', err.message);
    }
